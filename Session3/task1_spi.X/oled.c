...
void oled_clear_screen() {
    for (int line = 0; line < 4; line++) { oled_clear_line(line); }
}
/* Clear one line */
...
int oled_putchar4(char character,
                  FILE *stream) { // Use ASCII code minus 32, Font4
    if (character == '\n') {
        oled_pos(oled_current_line_num + 1,
                 8); // Next line with column margin of 8
        return 0;
    }
...
int oled_putchar5(char character,
                  FILE *stream) { // Use ASCII code minus 32, Font5
    if (character == '\n') {
...
int oled_putchar8(char character,
                  FILE *stream) { // Use ASCII code minus 32, Font8
    if (character == '\n') {
        oled_pos(oled_current_line_num + 1, 8);
        return 0;
    }
